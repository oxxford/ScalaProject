...
class HistoryEntrySpec extends AnyFlatSpec with Matchers with OptionValues {
  ...
  it should "should return None if input is incorrect" in {
    val input1 = "hour,10.0,1,1".split(',').toList
    val input2 = "-100,10.0,1,1".split(',').toList

    val input3 = "12,cpm,1,1".split(',').toList
    val input4 = "12,-10.0,1,1".split(',').toList

    val input5 = "12,10.0,bla,1".split(',').toList

    val input6 = "12,10.0,1,bla".split(',').toList
    ...
    HistoryEntry.fromLine(input5) shouldBe None
    HistoryEntry.fromLine(input6) shouldBe None
  }
